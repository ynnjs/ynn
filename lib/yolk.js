...
const YOLK_PORT = Symbol.for( 'yolk#port' );
const YOLK_KEYS = Symbol( 'yolk#keys' );
const YOLK_EXECUTE = Symbol( 'yolk#execute' );
const YOLK_REQUEST_BOOT = Symbol( 'yolk#request#boot' );
const YOLK_ACCESS_LOG = Symbol( 'yolk#access#log' );
 ...
 */
class Yolk extends BaseMixin( Koa ) {
    constructor( options = {} ) {
        super();
        this.setMaxListeners( 30 );
        if( is.undefined( options.debugging ) ) {
            if( argv.hasOwnProperty( 'debugging' ) ) {
                options.debugging = new Function( `return ${argv.debugging}` )();
            } else {
                options.debugging = this.parent ? this.parent.debugging : Console.DEFAULT;
            }
        }
        ...
        this[ INIT_CONFIG ]();
        if( !this.logger ) this[ INIT_LOGGER ]();

        if( !this.isModule ) {
            this.use( body( { multipart : true } ) );
            this.use( RSC.middleware() );
            this.use( this[YOLK_ACCESS_LOG]() );
        }
    ...
    listen( port ) {
        ...
        this.ready( () => {
            ...
            this[ YOLK_PORT ] = listen.address().port;

            this.console.spec( Array( 72 ).join( '-' ) );
            this.console.spec( 'Application is ready.' );
            this.console.spec( `Logs: ${this.logDir}` );
            this.console.spec( `Modules: [ ${Object.keys( this.modules || {} ).join( ', ' )} ].` );
            this.console.spec( `Configs: [ ${configItems.join( ', ' )} ].` );
            this.console.spec( `Controllers: [ ${Object.keys( this.controllers ).join( ', ' )} ].` );
            this.console.spec( `Services: [ ${Object.keys( this.services ).join( ', ' )} ].` );
            this.console.spec( `Plugins: [ ${Object.keys( this.plugins ).join( ', ' )} ].` );
            this.console.spec( `Port: ${this[YOLK_PORT]} - http://127.0.0.1:${this[YOLK_PORT]}.` );
    ...
    get keys() {
        let app = this, keys;
        while( app ) {
            if( app[ YOLK_KEYS ] ) keys = app[ YOLK_KEYS ];
            app = app.parent;
        }
        return keys;
    }
    ...
    [ YOLK_ACCESS_LOG ]() {
        return ( ctx, next ) => {
            ...
            const done = () => {
                let track = '-';
                if( ctx.yolkTrack && ctx.yolkTrack.length > 1 ) {
                    const t = [];
                    for( const item of ctx.yolkTrack ) {
                        t.push( item.step || '' );
                    }
                    track = t.join( '->' );
                }

                const msg = `${request.ip} - - [${new Date}] "${ctx.method}: ${ctx.yolkOriginalPath||request.path} ${request.protocol.toUpperCase()}/${req.httpVersion}" "${track}" ${res.statusCode} ${response.length} "${headers.referer||'-'}" "${headers['user-agent']||'-'}"`;
    ...
    [ INIT_CONFIG ]() {
        ...
        if( mounting.configDir ) {
            const config = this.loadFiles( mounting.configDir);
            if( config ) {
        ...
        /**
         * to load the default config files of Yolk
         */
    ...
    async [ INIT_LOGGER ]() {
        ...
        for( const level in config ) {
            ...
            if( item.console ) {
                transports.push( new winston.transports.Console( item ) );
            }
            if( item.filename ) {
                item.filename = path.resolve( this.logDir, item.filename );
            }
            
            if( item.dailyrotate ) {
                transports.push( new winston.transports.DailyRotateFile( item ) );
        ...
        if( !transports.length ) {
            this.console.log( 'no transports specified for logger.' ); 
    ...
    _initPlugins() {
        ...
        for( const name in plugins ) {
            ...
            try {
                plugin.instance = execPlugin( require( plugin.path ), name, this, plugin.options );
            } catch( e ) {
    ...
    async _initRouter() {
        this.router = new Router( this, ( matches, ctx ) => {
            ...
            if( matches && matches.yolkMounting ) {
    ...
    async _afterinit() {
        ...
        for( const module of modules ) {
            ...
            this.router.mount( rules, ctx => {
                if( !ctx.yolkOriginalPath ) {
                    ctx.yolkOriginalPath = ctx.path || '/';
                }
                if( !ctx.yolkTrack ) {
                    ctx.yolkTrack = [];
                }
    ...
    async [YOLK_EXECUTE]( ctx ) {
        const runtime = new Runtime( ctx, {
            console : this.console
        } );

        const boot = this[ YOLK_REQUEST_BOOT ];
        if( is.class( boot ) ) {
    ...
    async _initBoot() {
        ...
        try {
            this[ YOLK_REQUEST_BOOT ] = require( file );
        } catch( e ) {
    ...
    inspect() {
        ...
        //this.console.spec( Array( 72 ).join( '~' ) );
        //this.console.spec( `module "${this.name}" is ready.` );
        //this.console.spec( `logs: ${this.logDir}` );
        //this.console.spec( `modules: [ ${Object.keys( this.modules || {} ).join( ', ' )} ].` );
        //this.console.spec( `configs: [ ${Object.keys( this.config ).join( ', ' )} ].` );
        //this.console.spec( `controllers: [ ${Object.keys( this.controllers ).join( ', ' )} ].` );
        //this.console.spec( Array( 72 ).join( '-' ) );
    }
...
function execPlugin( plugin, name, app, options ) {
    if( is.class( plugin ) ) {
        return new plugin( app, options );
    } else if( is.function( plugin ) ) {
        return plugin( app, options );
    } else {
        throw new Error( `Plugin "${name}" should be a function or a constructor.` );
    }
}

Yolk.Console = Console;
