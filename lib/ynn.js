...
class Ynn extends BaseMixin( Koa ) {
    ...
    async _initControllers() {
        const dir = path.resolve( this.root, this.config( 'app.controller.dir' ) || 'controller' );
        if( utils.isdir( dir ) ) {
            try {
    ...
    async _initServices() {
        const dir = path.resolve( this.root, this.config( 'app.service.dir' ) || 'service' );
        if( utils.isdir( dir ) ) {
            try {
    ...
    async _initView() {
        ...
        for( const item of Object.keys( map ) ) {
            if( is.function( map[ item ].init ) ) {
                promises.push( map[ item ].init( this ) );
            }
        }
