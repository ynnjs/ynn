...
class Ynn extends BaseMixin( Koa ) {
    constructor( options = {} ) {
        ...
        if( is.undefined( options.debugging ) ) {
            if( cargs.hasOwnProperty( 'debugging' ) ) {
                ...
                options.debugging = this.parent ? this.parent.debugging : C.CONSOLE_DEFAULT;
            }
        }
     ...
     */
    async _initRouter() {
        this.router = new Router( this, ( matches, ctx, next ) => {
            if( !this.listenerCount( 'error' ) ) this.on( 'error', this.onerror );

            if( is.class( matches ) ) {
                return this[ EXECUTE_MIDDLEWARE ]( matches, ctx, next ); 
            }
