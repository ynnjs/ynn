...
class Ynn extends BaseMixin( Koa ) {
    constructor( options = {} ) {
        ...
        /**
         * to set app.proxy to true
         * so that the X-Forwarded-From header will be used for getting the IP address.
         */
    ...
    [ YNN_ACCESS_LOG ]() {
        return ( ctx, next ) => {
            ...
            const done = () => {
                ...
                const msg = `${request.ip} - - [${new Date}] "${ctx.method}: ${ctx.ynnOriginalPath||request.path} ${request.protocol.toUpperCase()}/${req.httpVersion}" "${track}" ${res.statusCode} ${response.length} "${headers.referer||'-'}" "${headers['user-agent']||'-'}"`;
    ...
    async sham( url, options, callback ) {
