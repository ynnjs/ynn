...
class Response {
    constructor( options ) {
        ...
        if( res ) {
            ...
            options.headers && Object.keys( options.headers ).forEach( ( name ) => {
                this.set( name, options.headers[ name ] );
            } );
            options.statusCode && ( this.status = options.statusCode );
            this.message = options.statusMessage === undefined ? statuses_1.default.message[ this.status ] ?? '' : options.statusMessage;
        }
    }
