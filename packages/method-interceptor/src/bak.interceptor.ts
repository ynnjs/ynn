...
import { KEY_BEFORE, KEY_AFTER, KEY_PARAMETER, KEY_EXCEPTION } from './constants';
import { MethodInterceptorMetadata, ParameterInterceptorMetadata, ExceptionInterceptorMetadata } from './metadata.interface';
import { Before, After, Exception, Parameters, MethodInfo, MethodPool } from './interceptor.interface';

type Pool = Record<string | symbol | number
...
export interface MethodsOptions {
    ...
    methodName: string,
    beforeMethods?: MethodPool;
    afterMethods?: MethodPool;
    parameterMethods?: MethodPool;
    exceptionMethods?: MethodPool;
}

export interface Methods<T extends any[]> {
    before: Before<T>;
    after: After<T>;
...
export function createMethodBefore<T>( descriptor: TypedPropertyDescriptor<any>, methods: InterceptorMethodPool | undefined ): Before<T> {
