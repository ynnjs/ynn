...
import {
    Context, Pipe, CommonRequestMetadata, CommonParameterMetadata, ResponseMetadata,
    runPipesInSequence, createGeneralDecorator, createResponseDecorator
} from '@ynn/common';
...
async function requestAndParameterInterceptor( metadata: CommonRequestMetadata | CommonParameterMetadata, ctx: Context ): Promise<unknown> {
    return runPipesInSequence(
        ...
        metadata.parameters.property ? ctx.request.get( metadata.parameters.property ) : ctx.request.headers,
        ctx,
        metadata
    );
}
