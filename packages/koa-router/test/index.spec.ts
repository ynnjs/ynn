...
describe( 'Router', () => {
    ...
    describe( 'static method Router.match', () => {
        ...
        it( 'should fill ctx.params with items matched in string', () => {
            const res = Router.match( '/a/:id', '/a/123' ) as any; // eslint-disable-line @typescript-eslint/no-explicit-any
            expect( res ).toHaveProperty( 'params', { id : '123' } );
            expect( res.matches.slice( 0, 2 ) ).toEqual( [ '/a/123', '123' ] );
        } );

        it( 'should return correct result with capturing groups', () => {
            const res = Router.match( /^\/a\/(?<id>\d+)/, '/a/123' ) as any; // eslint-disable-line @typescript-eslint/no-explicit-any
            expect( res ).toHaveProperty( 'params', { id : '123' } );
            expect( res.matches.slice( 0, 2 ) ).toEqual( [ '/a/123', '123' ] );
        } );
        ...
        it( 'should return correct result in complex situation', () => {
            ...
            const { matches, params } = res as any; // eslint-disable-line @typescript-eslint/no-explicit-any
