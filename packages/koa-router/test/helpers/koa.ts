...
import Koa, { KoaRequest, KoaResponse, KoaContext } from '@ynn/koa';
...
export class Req extends IncomingMessage {
    constructor( options: ReqOptions = {} ) {
        super( options.socket ?? new Stream.Duplex() as any ); // eslint-disable-line @typescript-eslint/no-explicit-any
         ...
         */
        ( this.socket as any ).remoteAddress = options.remoteAddress ?? this.socket.remoteAddress ?? '127.0.0.1'; // eslint-disable-line @typescript-eslint/no-explicit-any
    }
}
...
export function createContext( options: ContextOptions = {} ): KoaContext {
    const {
        app = new Koa(),
        req = new Req(),
        res = new Res()
...
export function createRequest( reqOrOptions?: IncomingMessage | ReqOptions ): KoaRequest {
    const req = reqOrOptions instanceof IncomingMessage ? reqOrOptions : new Req( reqOrOptions );
    return createContext( { req } ).request!; // eslint-disable-line @typescript-eslint/no-non-null-assertion
}

export function createResponse( resOrOptions?: ServerResponse | ResOptions ): KoaResponse {
    const res = resOrOptions instanceof ServerResponse ? resOrOptions : new Res( resOrOptions );
    return createContext( { res } ).response!; // eslint-disable-line @typescript-eslint/no-non-null-assertion
}

export default class extends Koa {
    async $( { req = new Req(), res = new Res() }: { req?: Req; res?: Res } ): Promise<unknown> {
